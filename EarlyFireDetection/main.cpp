...
namespace{
	...
	const char* InputVideoPath = "test_5.avi";
...
*/
//
//void writeContourPointsInfo(char ofInfoFileFolder[], Centroid & ctrd, const int frame, const int contour) { //NGAN
//
//	std::deque< std::vector< Feature > >::iterator itrDeq = ctrd.dOFRect.begin();
//
...
int main(void) {
	...
	cout<<sizeImg.width<< "x"<<sizeImg.height<<endl;
...
*/
	...
	while (key != 'x') {    // exit if user presses 'x' 
		...
		cvShowImage("imgDiff", imgDiff);
		cvShowImage("imgBackgroundModel", imgBackgroundModel);
		...
		//cvShowImage("imgStandardDeviation",imgStandardDeviation);
		
		cvShowImage("mask Motion", maskMotion);
		...
		//cvErode(maskHSI, maskHSI, maskMorphology, 7);
//#if defined( DEBUG_MODE ) && ( DEBUG_MODE == ON )
...
//#endif
		...
		/* Step6: Fire flow energy analysis*/
		// Pyramid Lucus-Kanade Optical Flow 
		...
		/* Step Froeground region analysis and Fire Dynamic behavior analysis*/
		...
		//cvWriteFrame(writer, imgFireAlarm);// save video output
